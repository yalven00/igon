  ...
  $.fn.validate = function() {
    return this.filter('form[data-validate]').each(function() {
      ...
      form
        ...
        .bind('form:validate:fail',   function(eventData) { $('[placeholder]').blur(); $("html").show_err("error", "Please fix errors and resubmit"); $('body').animate({
			         scrollTop: $(".field_with_errors").offset().top - 200
			     }, 10); clientSideValidations.callbacks.form.fail(  form, eventData); })
        .bind('form:validate:pass',   function(eventData) { $.noty.closeAll(); clientSideValidations.callbacks.form.pass(  form, eventData); })
    ...
    local: {
      ...
      confirmation: function(element, options) {
        //if (element.val() != jQuery('#' + element.attr('id') + '_confirmation').val()) {
			...
			if ((confirmationFieldValue != "Confirm Email" && confirmationFieldValue != "Confirm Password") && element.val() !== confirmationFieldValue) {
          return options.message;
        }
      }
    },
